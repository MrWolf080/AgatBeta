<?php
    ...
    if(!empty( $_FILES['image']['name'] ))
    {
        if ($_FILES['image']['error'] == 0)
        {
            if(substr($_FILES['image']['type'], 0, 5)=='image')
            {
                ...
                {
                    $_SESSION['bad_message']='Неизвестная ошибка при поиске id сотрудника';
                ...
                for($i=0;$i<count($ids, COUNT_RECURSIVE);$i++)
                {
                    ...
                    if(!$query)
                    {
                        $_SESSION['bad_message']='Неизвестная ошибка при создании связей с услугами';
                        header('Location: ../../admin/addservice.php');
                        exit;
                    }
                }
                $_SESSION['good_message']='Успешно добавлено';
                header('Location: ../../employers.php');
                exit;
            }
            else
            {
                $_SESSION['bad_message']='Загружен не графический файл';
                header('Location: ../../admin/addempl.php');
                exit;
            }
        }
        ...
        {
            $_SESSION['bad_message']='Неизвестная ошибка при загрузке файла';
            header('Location: ../../admin/addempl.php');
            exit;
        }
    }
    ...
    {
        $_SESSION['bad_message']='Файл не отправлен';
        header('Location: ../../admin/addempl.php');
        exit;
    }
