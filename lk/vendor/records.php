<?php
    ...
    if(!$_SESSION['user'])
    {
        $_SESSION['bad_message']="Записаться можно только авторизованным пользователям";
    ...
    if(mysqli_num_rows(mysqli_query($connect, "SELECT * FROM `record` WHERE `date`='$datetime' AND `id_user`='$id_user' AND `id_serv`='$id_serv' AND `id_empl`='$id_empl'"))==0)
    {
        ...
        {
            $_SESSION['bad_message']="Неизвестная ошибка записи";
            header("Location: ../../records.php");
            exit;
        }
        else
        {
            $_SESSION['good_message']="Вы успешно записаны";
            header("Location: ../../records.php");
            exit;
        }
    }
    ...
    {
        $_SESSION['bad_message']="Вы уже записаны";
        header("Location: ../../records.php");
        exit;
    }
